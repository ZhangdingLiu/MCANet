...
def per_class_metric(score_json, target_json, num_classes):
    """
    ...
    for index in range(num_classes):
        ...
        tp = np.sum((score >= 0.5) & (target == 1))
        fp = np.sum((score >= 0.5) & (target == 0))
        fn = np.sum((score < 0.5) & (target == 1))

        precision_per_class[index] = tp / (tp + fp) if (tp + fp) > 0 else 0
        recall_per_class[index] = tp / (tp + fn) if (tp + fn) > 0 else 0
        f1_per_class[index] = (2 * precision_per_class[index] * recall_per_class[index]) / \
                              (precision_per_class[index] + recall_per_class[index]) if (precision_per_class[index] + recall_per_class[index]) > 0 else 0
