...
class ResNet50_Multilabel(ResNet):
    ...
    def __init__(self, num_classes, depth=50, input_dim=2048, cutmix=None):
...
#         self.resnet.fc = nn.Sequential()  # Removing the original fc layer
#         self.classifier = nn.Linear(2048, num_classes)  # Adding a new fc layer
#
#     def forward(self, x):
#         x = self.resnet(x)
#         x = self.classifier(x)
